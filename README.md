...
6. **Preview/Import Consistency**: The preview (`/api/parse`) and import (`/api/import`) endpoints each independently re-parse and re-validate the uploaded file for implementation simplicity, rather than caching the validated dataset between steps. In a single-user demo context this is consistent, but in a concurrent environment the validation results between preview and import could diverge if the database state changes in between (e.g., another request inserts a matching email). A production implementation would cache the validated dataset — via session, temp table, or a validation token — between the preview and import steps to guarantee consistency.
